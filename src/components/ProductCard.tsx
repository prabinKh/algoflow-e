    ...
    >
      ...
      >
          ...
          <AnimatePresence>
            ...
            {product.rating >= 4.8 && (
              <motion.span 
                ...
                initial={{ scale: 0.8, opacity: 0, x: -10 }}
                animate={{ scale: 1, opacity: 1, x: 0 }}
                className="neo-badge bg-warning text-warning-foreground shadow-sm flex items-center gap-1"
              >
        ...
        <div className="product-card-image relative mb-3 overflow-hidden">
          ...
          {!product.inStock && (
            <div className="absolute inset-0 bg-background/60 backdrop-blur-[2px] flex items-center justify-center rounded-inner">
        ...
        <div className="flex-1 flex flex-col gap-2 px-1 pb-1">
          ...
          <div className="flex gap-1.5 flex-wrap">
            {product.specs.slice(0, 2).map((spec, i) => (
              <span key={i} className="px-2 py-0.5 bg-muted rounded text-[9px] uppercase tracking-wider font-semibold text-muted-foreground/80">
                {spec}
              </span>
            ))}
          </div>
